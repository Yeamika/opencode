  ...
  test("applies a line patch to a recently read file", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.metadata.files[0]).not.toHaveProperty("before")
        expect(result.metadata.files[0]).not.toHaveProperty("after")
      },
    })
  })
  ...
  test("applies a binary patch to a recently read binary file", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(result.metadata.files[0].diff).toContain("+++ target.bin")
        expect(result.metadata.files[0].diff).toContain("-Binary before: 5 bytes: 00 01 02 03 04")
        expect(result.metadata.files[0].diff).toContain("+Binary after:  7 bytes: FE 00 AA BB 03 CC DD")
        expect(result.metadata.files[0]).not.toHaveProperty("before")
        expect(result.metadata.files[0]).not.toHaveProperty("after")
      },
    })

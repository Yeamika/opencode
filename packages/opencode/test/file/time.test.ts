  ...
  describe("assert()", () => {
    ...
    test("matches remote file stamps by executor file key", async () => {
      ...
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          const file = { fileKey: "fid-1", canonicalPath: "/remote/file.txt", kind: "file" as const, size: 1, mtimeMs: 2 }
          await FileTime.read(sessionID, one, { executor: "remote", file })
          await FileTime.assert(sessionID, two, { executor: "remote", file })
          await expect(FileTime.assert(sessionID, two, { executor: "other", file })).rejects.toThrow("You must read file")
          await expect(
